...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        best_idx = 0
        total_samples = len(y)
        original_gini = 1 - np.sum((np.bincount(y) / total_samples) ** 2)
        ...
        for idx in possible_splits:
            ...
            left_gini = 1 - np.sum((np.bincount(y_left) / len(y_left)) ** 2)
            right_gini = 1 - np.sum((np.bincount(y_right) / len(y_right)) ** 2)

            weighted_gini = (len(y_left) * left_gini + len(y_right) * right_gini) / total_samples
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        if feature_subset is None or feature_subset > n_features:
