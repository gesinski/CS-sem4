...
%       z zastosowaniem zmiennych vpa.
    ...
    hold on;
    plot(x_fine, ya, 'b-', 'LineWidth', 1.5, 'DisplayName', 'Aproksymacja (double)');
    hold off;
    title('Aproksymacja - precyzja double');
    xlabel('x');
    ylabel('y');
    ...
    plot(x, y, 'k-', 'DisplayName', 'Dane oryginalne');
    hold on;
    plot(x_fine, yv, 'r-', 'LineWidth', 1.5, 'DisplayName', 'Aproksymacja (vpa)');
    hold off;
    title('Aproksymacja - precyzja vpa');
    xlabel('x');
    ylabel('y');
    legend('Location', 'best');
    axis([0 1 ymin ymax]);
