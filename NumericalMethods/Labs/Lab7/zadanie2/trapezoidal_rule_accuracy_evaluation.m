function [ft_5, integral_1000, Nt, integration_error] = trapezoidal_rule_accuracy_evaluation()
    ...
    for i = 1:(length(Nt))
        N = Nt(i);
        x = linspace(0, 5, N + 1); 
        integral_result = trapezoidal_rule(x);
...
function integral_approximation = trapezoidal_rule(x)
    ...
    for i = 2:(length(x)-1)  
        sum = sum + failure_density_function(x(i));
    end

    integral_approximation = dx * (0.5 * failure_density_function(x(1)) + sum + 0.5 * failure_density_function(x(end)));
end
